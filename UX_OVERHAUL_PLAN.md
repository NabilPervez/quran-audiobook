...
| 4: Bookmarks, notes and library | ✅ Done. One-tap bookmarks (player, desktop mini-player, B key, verse menu) with Undo; notes editor (from the toast, verse menu or Library); Library tab with Bookmarks (recent / by surah), Notes (searchable) and History; bookmark ticks on the seek bar; Recent bookmarks on Home; Settings page (skip intervals, auto-play next, reading prefs, export/import backup, reset progress, about). |
| 5: Offline and PWA | ✅ Done. Download manager (per surah, Juz ʿAmma, everything; progress, cancel, remove; audio + text; storage estimate; persistence request) served by the service worker with Range support; offline banner and dimmed unavailable rows; "Update available · Reload" prompt (no forced reloads mid-playback, hourly update checks); install card (native prompt / iOS instructions); TWA-ready manifest (id, categories, shortcuts, no orientation lock); privacy policy page; Netlify headers for `/.well-known/assetlinks.json`. |
